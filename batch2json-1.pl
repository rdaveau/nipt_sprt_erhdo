...
my($i, $j, $k, $fastq);
my $n = 1;
my @items = qw[ fastq genmode proband CI DPN DPNI NDP PDP HMZ HTZ SNP DIST ALPHA BETA ];
my @all_params = ();
my @new_params = ();
my @all_fastq = ();
my @new_fastq = ();
...
print TSV join("\t", qw[ design run_id fam_id matmut patmut genmode proband Mo Fa CI DPN DPNI NDP PDP HMZ HTZ SNP DIST ALPHA BETA target_gene analysis_id ]), "\n";
...
"matmut": "$readline{matmut}", "patmut": "$readline{patmut}", "target_gene": "$readline{target_gene}",
"sprt": [
!;
	...
	for $i (0 .. $#{$readline{genmode}}){

		@readline = ();
		push @readline, $readline{$_} foreach qw[ design run_id fam_id matmut patmut ];
		push @readline, ${$readline{$_}}[$i] foreach qw[ genmode proband ];
		push @readline, $fastq_ids{$_} foreach qw[ Mo Fa ];
		push @readline, $fastq_ids{${$readline{$_}}[$i]} foreach qw[ CI DPN DPNI ];
