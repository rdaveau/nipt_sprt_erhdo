...
my($i, $j, $k, $fastq);
my $n = 1;
my @items = qw[ fastq genmode proband CI DPNI NDP PDP HMZ HTZ SNP DIST ALPHA BETA ];
my @all_params = ();
my @new_params = ();
my @all_fastq = ();
my @new_fastq = ();
...
print TSV join("\t", qw[ design run_id fam_id matmut patmut genmode proband Mo Fa CI DPNI NDP PDP HMZ HTZ SNP DIST ALPHA BETA target_gene analysis_id ]), "\n";
