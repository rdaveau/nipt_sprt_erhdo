...
GetOptions(\%opts, 'genmode=s', 'proband=s', 'Mo=s', 'Fa=s', 'CI=s', 'DPNI=s', 'DPN=s', 'HMZ:f', 'HTZ:f', 'NDP:i', 'PDP:i', 'out=s');

foreach (qw[ Mo Fa CI DPNI DPN ]){
