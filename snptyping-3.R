...
for(ii in 1:length(chr)){
	...
	if(nrow(m)){
		ERROR.RATE[[paste0('chr', chr[[ii]])]] = data.frame(sapply(c('Mo', 'Fa', 'CI', 'DPNI'), function(x){
	...
	invisible(lapply(names(SNP3), function(chr){
		...
		lapply(setdiff(names(x), 'Ethnie'), function(NAME){
			...
			lapply(names(xx), function(SUBTYPE){
				...
				m = subset(xx[[SUBTYPE]], select = c(POS, DPNI.HAP, DPNI.VAF))
				if(nrow(m) >= opts$min.SNP){
					...
					p = ggplot(data = m) +
						...
						geom_tile(aes(x = 1:nrow(m), y = -.2, fill = DPNI.HAP), height = .15) +
						...
						geom_text(data = data.frame(x = nrow(m), y = -.2, label = 'NIPD'), aes(x = x, y = y, label = label), hjust = 0, vjust = .5, nudge_x = 1) +
