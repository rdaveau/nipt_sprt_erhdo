...
# Set inherired DPN haplotypes
if(substr(opts$gender, 1, 1) == 'M'){
	x$DPN.HAP = ifelse(x$DPN.GT == x$RISK.HAP, 1, 2)
}else{
	DPN.ALLELE = list(Fa = ifelse(x$Fa.GT == 'AA', 'A', 'B'))
	DPN.ALLELE$Mo = ifelse(x$DPN.GT == 'AA', 'A', ifelse(x$DPN.GT == 'BB', 'B', ifelse(DPN.ALLELE$Fa == 'A', 'B', 'A')))
	x$DPN.HAP = ifelse(DPN.ALLELE$Mo == x$RISK.HAP, 1, 2)
}
...
SPRT.TABLE = do.call(rbind, lapply(setdiff(names(x), c('Ethnie', 'ZFX')), function(NAME){
	...
	# Original 3-factors PC calculation (same as pc in snptyping_rxli-1.R)
	...
	# Original 3-factors CS calculation (same as c_score in snptyping_rxli-1.R)
	cs_3f = pc_3f$ALL$pc
