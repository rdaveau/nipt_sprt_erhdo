...
for fam_id in config["fam_ids"].keys():
	...
	for (RANK, sprt) in enumerate(config["fam_ids"][fam_id]["sprt"]):
		sprt_out.append(output_path + "/{}/{}/sprt/{RANK}/{genmode}/NDP{NDP}_PDP{PDP}_HMZ{HMZ}_HTZ{HTZ}_SNP{SNP}_DIST{DIST}_ALPHA{ALPHA}_BETA{BETA}/r-out/done".format(config["fam_ids"][fam_id]["design"], fam_id, RANK=RANK, **sprt))
...
rule sprt_in:
	input:
		...
		DPN = lambda wildcards: "{}/{}/{}/samples/{}/{}_final.pile".format(wildcards.prefix, wildcards.design, wildcards.fam_id, config["fam_ids"][wildcards.fam_id]["sprt"][int(wildcards.RANK)]["DPN"], config["fam_ids"][wildcards.fam_id]["sprt"][int(wildcards.RANK)]["DPN"]),
	...
	output:
		"{prefix}/{design}/{fam_id}/sprt/{RANK}/{genmode}/NDP{NDP}_PDP{PDP}_HMZ{HMZ}_HTZ{HTZ}_SNP{SNP}_DIST{DIST}_ALPHA{ALPHA}_BETA{BETA}/perl-out/done"
	log:
		"{prefix}/{design}/{fam_id}/sprt/{RANK}/{genmode}/NDP{NDP}_PDP{PDP}_HMZ{HMZ}_HTZ{HTZ}_SNP{SNP}_DIST{DIST}_ALPHA{ALPHA}_BETA{BETA}/logs/{fam_id}_sprt_in.log"
	shell:
		'snptyping-1.pl \
		--Mo {input.Mo} --Fa {input.Fa} --CI {input.CI} --DPN {input.DPN} --DPNI {input.DPNI} --proband {params.proband} \
		--genmode {wildcards.genmode} --NPD {wildcards.NDP} --PDP {wildcards.PDP} --HMZ {wildcards.HMZ} --HTZ {wildcards.HTZ} \
		--out {wildcards.prefix}/{wildcards.design}/{wildcards.fam_id}/sprt/{wildcards.RANK}/{wildcards.genmode}/NDP{wildcards.NDP}_PDP{wildcards.PDP}_HMZ{wildcards.HMZ}_HTZ{wildcards.HTZ}_SNP{wildcards.SNP}_DIST{wildcards.DIST}_ALPHA{wildcards.ALPHA}_BETA{wildcards.BETA}/perl-out/{wildcards.fam_id} \
		2>&1 | tee -a {log}'
...
rule sprt_out:
	input:
		...
		SPRT = "{prefix}/{design}/{fam_id}/sprt/{RANK}/{genmode}/NDP{NDP}_PDP{PDP}_HMZ{HMZ}_HTZ{HTZ}_SNP{SNP}_DIST{DIST}_ALPHA{ALPHA}_BETA{BETA}/perl-out/done"
	params:
		matmut = lambda wildcards: config["fam_ids"][wildcards.fam_id]["matmut"],
		patmut = lambda wildcards: config["fam_ids"][wildcards.fam_id]["patmut"],
		bedtarget = lambda wildcards: config["fam_ids"][wildcards.fam_id]["bed"],
		proband = lambda wildcards: config["fam_ids"][wildcards.fam_id]["sprt"][int(wildcards.RANK)]["proband"]
	output:
		"{prefix}/{design}/{fam_id}/sprt/{RANK}/{genmode}/NDP{NDP}_PDP{PDP}_HMZ{HMZ}_HTZ{HTZ}_SNP{SNP}_DIST{DIST}_ALPHA{ALPHA}_BETA{BETA}/r-out/done"
	log:
		"{prefix}/{design}/{fam_id}/sprt/{RANK}/{genmode}/NDP{NDP}_PDP{PDP}_HMZ{HMZ}_HTZ{HTZ}_SNP{SNP}_DIST{DIST}_ALPHA{ALPHA}_BETA{BETA}/logs/{fam_id}_sprt_out.log"
	shell:
		'snptyping-2.R \
		--ZFXY={input.DPNI} \
		--pref={wildcards.fam_id} \
		--idir={wildcards.prefix}/{wildcards.design}/{wildcards.fam_id}/sprt/{wildcards.RANK}/{wildcards.genmode}/NDP{wildcards.NDP}_PDP{wildcards.PDP}_HMZ{wildcards.HMZ}_HTZ{wildcards.HTZ}_SNP{wildcards.SNP}_DIST{wildcards.DIST}_ALPHA{wildcards.ALPHA}_BETA{wildcards.BETA}/perl-out \
		--odir={wildcards.prefix}/{wildcards.design}/{wildcards.fam_id}/sprt/{wildcards.RANK}/{wildcards.genmode}/NDP{wildcards.NDP}_PDP{wildcards.PDP}_HMZ{wildcards.HMZ}_HTZ{wildcards.HTZ}_SNP{wildcards.SNP}_DIST{wildcards.DIST}_ALPHA{wildcards.ALPHA}_BETA{wildcards.BETA}/r-out \
		--bed={params.bedtarget} --matmut={params.matmut} --patmut={params.patmut} --proband={params.proband} \
		--genmode={wildcards.genmode} --HTZ={wildcards.HTZ} \
